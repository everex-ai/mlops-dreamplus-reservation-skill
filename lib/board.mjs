...
/**
 * 예약 입력 정규화. 컴팩트 행 [roomCode, 'HH:mm', 'HH:mm', title?, memberId?, id?]을
 * 전체 객체로 확장(그리드처럼 큰 payload를 작게 넘길 때). 객체는 그대로 통과.
 */
export function expandReservations(items, date) {
  return items.map((it) => {
    if (!Array.isArray(it)) return it;
    const [roomCode, start, end, title = '', memberId = null, id = null] = it;
    return {
      roomCode,
...
/** 내(myId) 예약만 평탄화해 반환 (취소 대상 선택용) */
export function myReservations(board) {
  const out = [];
  for (const room of board.rooms) {
    for (const b of room.busy) {
      if (b.mine) {
        out.push({ id: b.id, roomCode: room.roomCode, name: room.name, start: b.start, end: b.end, title: b.title });
      }
    }
  }
